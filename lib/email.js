...
// Create SMTP transporter with Vercel-optimized settings
...
const verifyConnection = async () => {
  try {
    ...
    console.warn(
      '⚠️ SMTP verification failed, but will attempt to send emails:',
      error.message
    )
  }
}
...
export async function sendBookingConfirmationEmail(booking) {
  ...
  try {
    ...
    // Log booking details for manual follow-up
    console.log('📧 Manual follow-up needed for booking:', {
      id: booking.id,
      email: booking.email,
...
export async function sendPaymentConfirmationEmail(booking) {
  ...
  try {
    ...
    console.log('📧 Manual follow-up needed for payment confirmation:', {
      id: booking.id,
      email: booking.email,
      name: booking.full_name,
      package: booking.selected_package,
...
export async function sendAdminNotificationEmail(booking) {
    ...
    <body>
        <div class="container">
            ...
            <div class="content">
                ...
                }/admin" class="admin-link">View in Admin Dashboard</a>
            </div>
        </div>
    </body>
    </html>
  ...
  try {
    ...
    console.log('📧 Manual admin notification needed for booking:', {
      id: booking.id,
...
export async function sendStatusUpdateEmail(booking, oldStatus, newStatus) {
  ...
  try {
    ...
    console.log('📧 Manual status update needed for booking:', {
      id: booking.id,
      email: booking.email,
      name: booking.full_name,
